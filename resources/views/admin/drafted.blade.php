...
@section('content')
    <div class="container">
        <form action="{{ route('publish.drafted.books') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-dark mt-4" name="publishForm">Publish</button>
            ...
            @php
                ...
                foreach ($books as $book) {
                    ...
                    $btnDetails =
                        ...
                        '" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Details" target="_blank"><i class="fa fa-lg fa-fw fa-eye"></i></a>';
                    $btnDelete =
                        '<button type="button" class="btn btn-xs btn-default text-danger mx-1 shadow" title="Delete" onclick="showModal(' .
                        $book->id .
                    ...
                    if ($book->draft == 1) {
                        ...
                        $btnPublish =
                            ...
                            '" method="post" class="d-inline"><input type="hidden" name="_token" value="' .
                            csrf_token() . '"><button class="btn btn-xs btn-default text-info mx-1 shadow" title="Publish" type="submit" name="publishForm">
                            <i class="fa fa-lg fas fa-upload"></i></button></form>';
                    } else {
                        $btnPublish = '';
                        $btnDraft =
                            '<form action="' .
                    ...
                    $recored = ['<input type="checkbox" name="drafted[]" value="' . $book->id . '">', $book->id, $book->title, substr($book->author, 2), $book->draft == 1 ? 'Drafted' : 'Published', '<nobr>' . $btnDraft . $btnPublish . $btnEdit . $btnDelete . $btnDetails . '</nobr>'];
